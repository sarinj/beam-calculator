...
function addFootingContentDetailed(
  ...
  const As_min_x = rho_min * bx * h * 100;
  doc.text(`As,req = rho x b x d = ${As_req_x.toFixed(2)} cm²`, col2X + 4, rightY);
  rightY += 3.5;
  doc.text(`As,min = ${rho_min} x b x h = ${As_min_x.toFixed(2)} cm²`, col2X + 4, rightY);
  rightY += 3.5;
  doc.setFont('helvetica', 'bold');
  ...
  const As_min_y = rho_min * by * h * 100;
  ...
  doc.setFont('helvetica', 'normal');
  doc.setFontSize(7.5);
  const x_shear_x = lx - d;
  const x_shear_y = ly - d;
  const Vu_x = qu * B * Math.max(0, x_shear_x);
  const Vu_y = qu * B * Math.max(0, x_shear_y);
  const Vc_beam = 0.85 * 0.17 * Math.sqrt(fc) * B * 100 * d * 100 / 1000;
  doc.text('X-direction:', col2X + 4, rightY);
  rightY += 3.5;
  doc.text(`x = (B - c1) / 2 - d = ${x_shear_x.toFixed(3)} m`, col2X + 7, rightY);
  rightY += 3.5;
  doc.text(`Vu = qu x B x x = ${Vu_x.toFixed(2)} Tonf`, col2X + 7, rightY);
  rightY += 3.5;
  doc.text('Y-direction:', col2X + 4, rightY);
  rightY += 3.5;
  doc.text(`x = (B - c2) / 2 - d = ${x_shear_y.toFixed(3)} m`, col2X + 7, rightY);
  rightY += 3.5;
  doc.text(`Vu = qu x B x x = ${Vu_y.toFixed(2)} Tonf`, col2X + 7, rightY);
  rightY += 3.5;
